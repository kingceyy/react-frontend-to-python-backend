...
async def get_current_user(
    ...
    user_id = crud.verify_jwt_token(token) if hasattr(crud, 'verify_jwt_token') else None
    
    # Import directly since it's in auth
...
async def auth_telegram(
    ...
    if request.start_param and request.start_param.startswith("ref_"):
        ...
        if referrer and user.referrer_id is None:
...
async def get_coupons_history(
    ...
    stmt = select(crud.CouponUsage).where(
        crud.CouponUsage.user_id == user.telegram_id
    )
...
async def get_leaderboard(
    ...
    """Get leaderboard"""
    from sqlalchemy import select
    stmt = select(crud.User).order_by(crud.User.active_invites.desc()).limit(100)
    result = await session.execute(stmt)
...
async def get_referees(
    ...
    from sqlalchemy import select
    stmt = select(crud.User).where(crud.User.referrer_id == user.id)
    result = await session.execute(stmt)
    ...
    return [
        Referee(
            ...
            active=True,  # Simplified - could track actual activity
        )
        for ref in referees
    ]
...
async def forcesub_check(
    ...
    # This would require bot API call
    # For now, return True (implement with aiogram getChatMember in bot)
...
async def admin_validate_coupon(
    ...
    success = await crud.validate_coupon(session, coupon_id, user.telegram_id)
    if not success:
...
async def admin_get_coupons(
    ...
    from sqlalchemy import select
    stmt = select(crud.Coupon).order_by(crud.Coupon.created_at.desc())
    result = await session.execute(stmt)
...
async def admin_update_user(
    ...
    updated = await crud.update_user(session, user_id, **payload)
    if not updated:
        raise HTTPException(status_code=404, detail="User not found")
